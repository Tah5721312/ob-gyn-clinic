...
export default function PaymentsClient() {
  ...
  const handleConfirmRefund = async () => {
    ...
    try {
      const response = await apiFetch(
        `/api/payments/${selectedPayment.id}/refund`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const totalAmount = payments.reduce(
    (sum, p) => sum + (p.isRefunded ? 0 : p.amount),
    0
  );
  const refundedAmount = payments.reduce(
    (sum, p) => sum + (p.isRefunded ? p.amount : 0),
    0
  );
                  ...
                  >
                    ...
                    <td className='px-6 py-4 whitespace-nowrap'>
                      <span
                        className={`font-semibold ${
                          payment.isRefunded ? 'text-red-600' : 'text-green-600'
                        }`}
                      >
                    ...
                    <td className='px-6 py-4 whitespace-nowrap'>
                      <div className='flex items-center gap-2'>
                        <button
                          onClick={(e) =>
                            router.push(`/billing/${payment.invoiceId}`)
                          }
          ...
          >
            ...
            <div className='p-6'>
              <p className='text-gray-700 mb-4'>
                هل أنت متأكد من حذف الدفعة رقم{' '}
